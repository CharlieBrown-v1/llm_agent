"""From https://github.com/allenai/open-instruct/blob/main/eval/utils.py."""
...
lumos_dir_path = Path(__file__).parent.parent
test_model_path = lumos_dir_path.parent.joinpath('.cache/models--gpt2/snapshots/11c5a3d5811f50298f278a704980280950aedb10')
...
@torch.no_grad()
def rl_generate_completions(rl_model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, disable_tqdm=False, **generation_kwargs):
    generations = []
    if not disable_tqdm:
        progress = tqdm.tqdm(total=len(prompts), desc="Generating Completions")
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        batch_input_ids = batch_input_ids.to(rl_model.pretrained_model.device)
        attention_mask = attention_mask.to(rl_model.pretrained_model.device)

        # try:
        batch_outputs = rl_model.generate(
            input_ids=batch_input_ids,
            attention_mask=attention_mask,
